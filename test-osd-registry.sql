...
select osd_id,submission_id,start_lat_verb from samples 
 where local_date = '2014-06-18'::date and local_start between '20:00:00' and '21:31:00';
--*/

UPDATE osdregistry.samples
   SET (start_lat,start_lon,stop_lat,stop_lon,curator,curation_remark) 
     = (start_lat_verb::numeric,start_lon_verb::numeric,stop_lat_verb::numeric,stop_lon_verb::numeric,'rkottman', 'mass systematic update')
 WHERE start_lat_verb NOT IN ('33.55','51.434733','+43.22639','59.8822','45.548833','27.02527','43.8604','+53.225417','45.314350','-45.7442','+054.8333','21.4934','+44.6936','41.1416','33.32306','43.63871944444445', '59.6220', '59.89961', '59.81618')
...
CREATE FUNCTION ena_sample_attribute(k text, v text, unit text )
  ...
  SELECT CASE WHEN v IS NOT NULL OR v <> '' THEN
...
SELECT 
       xmlelement(name "SAMPLE", xmlattributes( label as "alias"),
          xmlelement(name "DESCRIPTION", objective),
          xmlelement(name "SAMPLE_ATTRIBUTES", 
             ena_sample_attribute('SAMPLING_Campaign', 'OSD-Jun-2014' ),
-- todo add site name 
             ena_sample_attribute('SAMPLING_Site', osd_id || ', add site name' ),
             ena_sample_attribute('SAMPLING_Platform', platform ),
             ena_sample_attribute('EVENT_TIME', local_date || 'T' || local_start ),
             ena_sample_attribute('EVENT_Longitude', start_lon::text ),
             ena_sample_attribute('EVENT_Latitude', start_lat::text ),
             ena_sample_attribute('SAMPLE_DEPTH', water_depth::text ),
             ena_sample_attribute('SAMPLE_Protocol_Label', protocol::text ),
             ena_sample_attribute('SAMPLE_Title', label ),
             ena_sample_attribute('ENVIRONMENT_Biome', biome ),
             ena_sample_attribute('ENVIRONMENT_Feature', feature ),
             ena_sample_attribute('ENVIRONMENT_Material', material ),
             ena_sample_attribute('ENVIRONMENT_Temperature', temperature, '°C' ),
             ena_sample_attribute('ENVIRONMENT_Salinity', salinity, 'PSU'),

             ena_sample_attribute('SAMPLING_Project', 'Micro B3' ),
