...
INSERT INTO core.isolates (strain, num_chromosomes, num_plasmids, study, sample_name, attr)
  ...
  FROM
  (
    SELECT DISTINCT organism_name as lab FROM ncbi.sequences_unformatted_mat
    EXCEPT
    SELECT label as lab FROM core.isolates
  ) AS a
  INNER JOIN
  (
...
INSERT INTO core.genome_studies (label, isolate_name, gpid)
  SELECT 'ncbi-bacterial-genomes' as label, 
         trim(organism_name) as isolate_name,
         0 AS gpid
  FROM
  (
    SELECT DISTINCT organism_name FROM ncbi.sequences_unformatted_mat
  ) AS a
;
