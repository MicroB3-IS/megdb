...
BEGIN;
SELECT _v.register_patch('00155-myosd-fix-postal-code-insert-bug',
                          array['00154-myosd-gsheet-view'] );

-- section of creation best as user role megdb_admin
SET ROLE megdb_admin;


--need to fix bug in postal code of stations previous function wrongly
-- inserted dhl station in postal_code

UPDATE myosd.registrations
   SET postal_code = CASE raw->>'post_station'	
                    WHEN 'post' THEN raw#>>'{address,poastal_code}'
                    WHEN 'station' THEN raw#>>'{station,poastal_code}'
		    ELSE ''
                  END
  
   
 WHERE post_station = 'station' returning *;
...
VALUES (
  ...
  CASE in_raw->>'post_station'
    ...
    WHEN 'station' THEN in_raw#>>'{station,poastal_code}'
    ELSE ''
  END,
  CASE in_raw->>'post_station'
