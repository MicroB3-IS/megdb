...
BEGIN;
SELECT _v.register_patch('00148-osdregistry-better-sample-data-view.sql',
                          array['00147-osdregistry-add-lme-25km-tagging'] );
...
CREATE OR REPLACE VIEW osdregistry.sample_environmental_data AS 
 SELECT sam.osd_id, 
   ...
   FROM osdregistry.samples sam
        LEFT JOIN osdregistry.iho_25km_tagging iho
	...
	LEFT JOIN osdregistry.sample_boundaries_tagging bt
        ...
        LEFT JOIN osdregistry.sample_longhurst_36min_tagging lh
          ON sam.submission_id = lh.submission_id
	LEFT JOIN osdregistry.sample_lme_25km_tagging lme
